...
edited = st.data_editor(
    ...
    column_config={
        ...
        "Term": st.column_config.SelectboxColumn(
            ...
            required=False,
            help="S1 uses Q1+Q2+E1. S2 uses Q3+Q4+F1. Unneeded cells will clear automatically.",
        ),
        ...
        "Q4 %": st.column_config.NumberColumn("Q4 %", min_value=0.0, max_value=100.0, step=0.01, format="%.2f"),
        "F1 %": st.column_config.NumberColumn("F1 %", min_value=0.0, max_value=100.0, step=0.01, format="%.2f"),
        "Course %": st.column_config.NumberColumn("Course %", min_value=0.0, max_value=100.0, step=0.01, format="%.2f"),
        "Remainder %": st.column_config.NumberColumn("Remainder %", min_value=0.0, max_value=100.0, step=0.01, format="%.2f"),
    },
    hide_index=True,
)
